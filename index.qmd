...
posterior      = estimate(specification, S = 1000, show_progress = FALSE)
```
...
where shock 1 is the optimism shock. Comparing with the US data, the optimism shock has a much smaller effect on consumption and hours worked in Australia, only stock prices has a significant effect in the short run.

### FEVD

## Extension

Now we impose an additional narrative restrictions that
...
sign_irf          = array(0, dim = c(5, 5, 1))
sign_irf[2, 1, 1] = 1
sign_narrative    = matrix(c(1, -1, 1, 1, 103, 0), ncol = 6)
