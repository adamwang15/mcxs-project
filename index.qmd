...
where $\mathcal Z$ denotes zero restrictions and $\mathcal S$ denotes sign restrictions. The numerator is the target density, and the denominator is the proposal density from steps 1-2. Here is the code to calculate the volume element $v_{(g\circ f_h)|\mathcal Z}$:
