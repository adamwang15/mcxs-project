...
To perform this identification, we need an algorithm to sample from the posterior distribution of the structural parameters ($\*A_0$, $\*A_+$) conditional on the zero and sign restrictions.
...
To be explicit, starting with a Uniform-Normal-Inverse-Wishart prior distribution, the importance weight in step 3 is given by

$$
\begin{align*}
\frac{\mathcal{NGN}(\*A_0,\*A_+|\mathcal Z, \mathcal S)}{\mathcal{UNIW}(\*B,\*\Sigma,\*Q|\mathcal Z)v_{(g\circ f_h)|\mathcal Z}(\*A_0,\*A_+)}
&=\frac{\mathcal{UNIW}(\*B,\*\Sigma,\*Q|\mathcal Z, \mathcal S)v_{f_h}(\*A_0,\*A_+)}{\mathcal{UNIW}(\*B,\*\Sigma,\*Q|\mathcal Z)v_{(g\circ f_h)|\mathcal Z}(\*A_0,\*A_+)}\\
&\propto\frac{|\text{det}(\*A_0)|^{-(2N+K+1)}}{v_{(g\circ f_h)|\mathcal Z}(\*A_0,\*A_+)}
\end{align*}
$$

where $\mathcal Z$ denotes satisfying zero restrictions and $\mathcal S$ denotes satisfying the sign restrictions. The numerator is the target density, and the denominator is the proposal density from steps 1-2. Here is the code to calculate this importance weight.
...
To combine these two identification schemes, we need to calculate the importance weight when **all** of the three restrictions, zero, sign, and narrative, are present:

$$
\begin{align*}
\frac{\mathcal{NGN}(\*A_0,\*A_+|\mathcal Z, \mathcal S, \mathcal R)}{\mathcal{UNIW}(\*B,\*\Sigma,\*Q|\mathcal Z)v_{(g\circ f_h)|\mathcal Z}(\*A_0,\*A_+)}
&=\frac{\mathcal{UNIW}(\*B,\*\Sigma,\*Q|\mathcal Z, \mathcal S, \mathcal R)v_{f_h}(\*A_0,\*A_+)}{\mathcal{UNIW}(\*B,\*\Sigma,\*Q|\mathcal Z)v_{(g\circ f_h)|\mathcal Z}(\*A_0,\*A_+)}\\
...
Therefore, it is simply the product of the importance weight for the narrative restrictions and the importance weight for the zero and sign restrictions.
...
$$
\text{FEVD}_j^i(h) = \frac{\text{MSFE}_{j}^{i}(h)}{\sum_{n=1}^{N}\text{MSFE}_{j}^{n}(h)}, \quad \text{MSFE}_{j}^{i}(h) = \sum_{l=0}^{h-1} \*\Theta_{ij,l}
$$
...
```{r}
fevd = compute_variance_decompositions(posterior, horizon = 40)
```
...
sign_irf          = array(0, dim = c(5, 5, 1))
sign_irf[2, 1, 1] = 1
sign_narrative    = matrix(c(1, -1, 1, 1, covid_index, 0), ncol = 6)
...
With the additional assumption, we can identify more significant impacts. In the short run, the optimism shock has a positive impact on consumption and a negative impact on hours worked.

Although not significant, it is interesting to see that the optimism shock has a negative impact on hours worked in Australia in the short run, in contrast to a positive impact in the US.
